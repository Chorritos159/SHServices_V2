#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Carga Real Mixta Multiproceso (Lecturas + Escrituras) - 1 Millón de peticiones.
...
Uso:
    python pruebas_reales/carga_1M.py
"""
import argparse
import glob
import json
...
def main():
    ...
    ap.add_argument("--total", type=int, default=1000000, help="Peticiones totales a completar")
    ap.add_argument("--nodos", type=int, default=160, help="Nodos concurrentes totales")
    ...
    ap.add_argument("--tope-min", type=int, default=35, help="Tope de seguridad en minutos")
    args = ap.parse_args()
        ...
        for i in range(args.procesos):
            ...
            p = subprocess.Popen(
                [sys.executable, os.path.join(LIB, "carga_nodos.py"),
                 ...
                 "--objetivo", "1M-real-multi",
                 "--usuario", "admin", "--password", "admin123",
                 "--nombre", f"carga_1M_real_{marca}", "--salida", RESULTADOS,
    ...
    for marca in marcas_temporales:
        encontrados = glob.glob(os.path.join(RESULTADOS, f"carga_1M_real_{marca}_*.json"))
    ...
    banner("REPORTE CONSOLIDADO MULTIPROCESO (1M)")
