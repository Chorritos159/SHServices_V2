#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Carga Real Mixta Multiproceso (Lecturas + Escrituras) - 100 000 peticiones.
...
def main():
    ...
    ap.add_argument("--total", type=int, default=100000, help="Peticiones totales a completar")
    ap.add_argument("--nodos", type=int, default=96, help="Nodos concurrentes totales")
    ...
    ap.add_argument("--tope-min", type=int, default=15, help="Tope de seguridad en minutos")
    args = ap.parse_args()
        ...
        for i in range(args.procesos):
            ...
            p = subprocess.Popen(
                [sys.executable, os.path.join(LIB, "carga_nodos.py"),
                 ...
                 "--objetivo", "100k-real-multi",
                 "--usuario", "admin", "--password", "admin123",
                 "--nombre", f"carga_100k_real_{marca}", "--salida", RESULTADOS,
    ...
    # Intentar consolidar reportes JSON generados
    print("\nConsolidando reportes de los procesos paralelos...")
    total_enviadas = 0
    total_exitosas = 0
    codigos_combinados = {}
    
    # Buscamos los archivos JSON recién creados que contengan las marcas
    archivos_json = []
    for marca in marcas_temporales:
        encontrados = glob.glob(os.path.join(RESULTADOS, f"carga_100k_real_{marca}_*.json"))
    ...
    # Sumamos éxitos (códigos < 400 y numéricos)
    ...
    banner("REPORTE CONSOLIDADO MULTIPROCESO (100K)")
