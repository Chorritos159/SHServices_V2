...
| 10 | Los microservicios confían en las cabeceras `X-User-*` del Gateway sin re-validar el JWT (OWASP A01) | Quien alcance la red Docker interna puede falsificar identidad/rol. Mitigado: ningún microservicio de negocio publica puerto al host | Propagar el JWT y validarlo también en cada servicio (defensa en profundidad), o malla con mTLS | Owner técnico de Seguridad |
