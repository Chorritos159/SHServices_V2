...
| `v1.0` | Adaptación a V2 (delegando facturación y diagnóstico a nuevos ms) | Breaking | Cambiar flujos a nuevos microservicios |
| `v1.1` | feat(resiliencia S34, Fase 3): idempotencia en `POST /tickets` vía cabecera `Idempotency-Key` opt-in — un reintento con la misma clave devuelve el ticket ya creado en vez de duplicarlo (no hay clave natural: el mismo cliente puede traer el mismo equipo en visitas legítimas distintas). Logs migrados al formato mínimo S34 | Compatible | Opcional: enviar `Idempotency-Key` para que los reintentos sean seguros |
| `v1.2` | fix(observabilidad): el servicio se identificaba como `ticket_service` (guion bajo) en sus logs, mientras los otros 8 usan guion (`ticket-service`) — rompía la consistencia del formato que exige la S34 y obligaba a filtrar por dos nombres distintos en Loki | Compatible | Ninguna (si filtrabas logs por `service="ticket_service"`, ahora es `ticket-service`) |
| `v1.5` | refactor(dominio): **se retiran las garantias** de este servicio (pasan a facturacion, ADR-0013): `/entregar` ya solo confirma el stock y cierra el ticket; se eliminan los endpoints `GET /garantias` y `/garantias/por-documento`. fix(carga): el ID de ticket usaba 4 hex (65.536 combos) y colisionaba bajo concurrencia devolviendo 500 (UniqueViolation) -> ahora 8 hex + reintento con ID nuevo | **Ruptura** (endpoints de garantias movidos) | Consultar garantias en `/api/v1/facturas/garantias/` |
