...
| :--- | :--- | :--- | :--- |
| `v1.0` | Creación inicial del servicio en la arquitectura V2 | Release | Ninguna (consumidor pasivo) |
| `v1.1` | feat(resiliencia S34, Fase 3): idempotencia del consumidor — índice único `(trace_id, evento)` evita duplicar un registro de traza ante un redelivery de RabbitMQ (ack perdido tras persistir). Logs migrados al formato mínimo S34 (`service, correlationId, operation, event, result, durationMs`) | Compatible | Ninguna |
...
| `v1.3` | fix(fiabilidad, hallazgo de SonarQube): la tarea del consumidor de RabbitMQ se lanzaba con `asyncio.create_task()` sin guardar la referencia — el event loop solo mantiene referencias débiles, así que el garbage collector podía recolectarla a medio camino y el servicio dejaría de auditar eventos **en silencio**. Ahora la referencia se guarda a nivel de módulo | Compatible | Ninguna |
| `v1.2` | fix(auditoria): la cola tenia un solo binding `ticket.*`, asi que `producto.registrado` se publicaba pero NUNCA se auditaba (brecha #3). Se anade un segundo binding `producto.*`; los logs muestran la entidad correcta (idTicket o codigo de producto) | Compatible | Ninguna: ahora la traza incluye los movimientos de inventario |
